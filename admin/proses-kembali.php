<?php
	...
	if(isset($_GET['id'])){
		...
		$nm_peminjam		  		  = $data_req_kembali['peminjam'];
		$level			  		  = $data_req_kembali['level'];
		$no_tlp					  = $data_req_kembali['no_tlp'];
		...
		if($data_search_barang){
			...
			if($update_stok){
				if(mysql_query("INSERT INTO tbl_transaksi (nama_barang, peminjam, level, no_tlp, jml_barang, tgl_pinjam, tgl_kembali) VALUES ('$nama_barang', '$nmpeminjam', '$level', '$no_tlp', '$jml_barang', '$tgl_pinjam', '$tgl_kembali')")){
